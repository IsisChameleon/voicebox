...
- Last full describe pass: 2026-08-03; generated-utterance path refreshed 2026-08-12 for issue #19.
...
| Stock `KokoroTTSService` | Local TTS (`af_heart`) configured with TOKEN aggregation so one `speak()` is one synthesis context | `src/voicebox/agent.py:981-985` |
| `GeneratedUtteranceAudioBuffer` | Tester-side playout policy: holds generated audio chunks until TTS completes, releases one contiguous span, and discards partial audio on error/interruption | `src/voicebox/processors/generated_utterance_audio_buffer.py:18-61` |
...
| Generated utterance playout | Stock Kokoro may produce delayed chunks; `GeneratedUtteranceAudioBuffer` waits for `TTSStoppedFrame`, then pushes the saved audio frames back-to-back before forwarding the stop. Error, interruption, cancel, and end discard unplayed audio. | `src/voicebox/processors/generated_utterance_audio_buffer.py:37-61`, placement `src/voicebox/agent.py:1025-1032` |
...
| Model artifacts | Stock pipecat Kokoro manages its ONNX cache; Whisper `Systran/faster-distil-whisper-large-v3` runs on CPU int8 (CUDA deliberately not required) | Kokoro construction `src/voicebox/agent.py:981-985`, dependency `pyproject.toml:25-30`, Whisper `src/voicebox/agent.py:970-979` |
...
| I4 | One `speak()` is one TOKEN-aggregated synthesis context whose generated audio reaches the transport as one buffered, gap-free span; partial audio is discarded on failure/interruption | Synthesis gaps became 1.6–4.2 s of real mic silence; the app heard one utterance as several turns. Playing a failed/interrupted prefix would resurrect an invalid turn. | `src/voicebox/agent.py:981-985`, `src/voicebox/processors/generated_utterance_audio_buffer.py:37-61` | `tests/test_generated_utterance_audio_buffer.py`, `tests/test_kokoro_playout.py`, S3 |
...
| 5 | LLM-response frame triplet queued → VAD and STT pass it through → stock Kokoro synthesizes one TOKEN context → `GeneratedUtteranceAudioBuffer` releases one contiguous span → retained assistant aggregator records tester context | logical | `src/voicebox/agent.py:_queue_speak_frames`, `src/voicebox/agent.py:981-985`, `src/voicebox/agent.py:1025-1032`, `src/voicebox/processors/generated_utterance_audio_buffer.py:37-61` |
