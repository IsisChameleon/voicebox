...
- Last full describe pass: 2026-08-03; generated-utterance path refreshed 2026-08-14 for issues #19/#24
  (delta: `deltas/generated-utterance-audio-buffer.md`).
...
(`src/voicebox/events.py:9-15`) and the pipeline observer (`src/voicebox/agent.py:388-395`).
...
| Stock `KokoroTTSService` | Local TTS (`af_heart`) configured with TOKEN aggregation so one `speak()` is one synthesis context | `src/voicebox/agent.py:984-989` |
| `GeneratedUtteranceAudioBuffer` | Tester-side playout policy: holds generated audio chunks until TTS completes, releases one contiguous span, and discards still-held audio on interruption (and on an error raised below it) | `src/voicebox/processors/generated_utterance_audio_buffer.py:18-79` |
...
| `compute_metrics` | Pure function: event log in → `metrics.json` report out (latencies, talk-over, dead air vs think time vs outage, turns) with explicit bias notes | `src/voicebox/metrics.py:15-78`, biases `src/voicebox/metrics.py:166-194` |
...
pipecat (`src/voicebox/server.py:24-26`, `src/voicebox/server.py:41-59`). `bot.py` bridges IPC
to `PipecatMCPAgent` (`src/voicebox/bot.py:49-50`). UI driving is *not* in this system: an
...
|---|---|---|
| Process creation | `multiprocessing` with `spawn` forced (fork from async context copies loop/fds/locks) | `src/voicebox/agent_ipc.py:22-25`, `src/voicebox/agent_ipc.py:92-96`; browser child `src/voicebox/browser_session.py:76-93` |
...
| Child command loop | Every command runs as its own asyncio task — a `speak` arriving during a blocked `listen` executes immediately; `stop` breaks the loop | `src/voicebox/bot.py:85-111` |
| Deadlines | Parent-side per-command deadlines sized to outlive the child's internal budgets: `stop`=210 s (> drain cap 180 s), `speak` waits=60 s (> playout timeout 30 s), `listen`=timeout+30 s | `src/voicebox/server.py:41-59`, `src/voicebox/server.py:62-88`, `src/voicebox/server.py:274-276` |
| Audio plane | Out-of-band raw-PCM WebSocket between shim and pipecat — audio never crosses the MCP boundary | `src/voicebox/agent.py:1034-1046`, `src/voicebox/shim.js:94-140` |
...
| Generated utterance playout | Stock Kokoro may produce delayed chunks; `GeneratedUtteranceAudioBuffer` waits for `TTSStoppedFrame`, then pushes the saved audio frames back-to-back before forwarding the stop. Interruption, cancel, end, and an error raised BELOW the buffer discard unplayed audio; a TTS synthesis failure does NOT reach it (Gaps, issue #25). | `src/voicebox/processors/generated_utterance_audio_buffer.py:37-79`, placement `src/voicebox/agent.py:1034` |
| Event-log sync | `asyncio.Condition`; `listen_events` blocks until the log grows past the cursor; `speak(wait_for_turn)` blocks on the same condition until `_app_bot_speaking` clears | `src/voicebox/agent.py:297-304`, `src/voicebox/agent.py:716-731`, `src/voicebox/agent.py:879-882` |
| Armed barge-in | Background task per `speak(when=...)`; log position snapshotted synchronously at arm time so only later events trigger; all armed tasks cancelled on stop | `src/voicebox/agent.py:816-828`, `src/voicebox/agent.py:884-904`, `src/voicebox/agent.py:562-566` |
...
| Teardown ordering | `session_stopped` emitted first (wakes pending listens) → cancel armed tasks → STT drain (budget scales with backlog) → settle log → dump artifacts → `EndFrame` → runner returns → parent reaps both children | `src/voicebox/agent.py:542-597`, `src/voicebox/bot.py:90-107`, `src/voicebox/server.py:393-415` |
...
data and injected as text (`src/voicebox/browser_session.py:273-274`).
...
| Voice pipeline | `agent.py`, stock pipecat Kokoro, `runner_args.py`, `raw_pcm_serializer.py`, `processors/` (generated-utterance buffering, TTS warm-up, non-blocking Whisper), `timing.py` | `src/voicebox/agent.py:7-21`, `src/voicebox/processors/generated_utterance_audio_buffer.py`, `src/voicebox/processors/tts.py` |
...
| Unit tests | `tests/` — pytest-asyncio auto mode; pin the traps: generated-utterance ordering/failure paths (`tests/test_generated_utterance_audio_buffer.py`), stock Kokoro + playout completion (`tests/test_kokoro_playout.py`), VAD placement (`tests/test_vad_placement.py`), non-blocking STT (`tests/test_nonblocking_stt.py`), stop-drains-STT (`tests/test_stop_drains_stt.py`), speak surface, metrics, browser startup, and timing | `pyproject.toml:79-81` |
...
| pipecat child | WebSocket server `localhost:9091` (raw PCM) | `src/voicebox/server.py:186-191`, `src/voicebox/runner_args.py:43-44` |
...
| Model artifacts | Stock pipecat Kokoro manages its ONNX cache; Whisper `Systran/faster-distil-whisper-large-v3` runs on CPU int8 (CUDA deliberately not required) | Kokoro construction `src/voicebox/agent.py:984-989`, dependency `pyproject.toml:25-30`, Whisper `src/voicebox/agent.py:970-979` |
| Run artifacts | `record_dir` (repo convention: `temp/<run>/`) — WAVs, `events.json`, `metrics.json`, `agent-debug.log` (pipecat child) plus `shim.log`, `shim_diag.json` (browser child) | `src/voicebox/agent.py:444-452`, `src/voicebox/agent.py:614-685`, `src/voicebox/browser_session.py:183-224` |

Ports are preflighted so a second session fails with a retryable error instead of colliding
(`src/voicebox/server.py:91-121`, `src/voicebox/server.py:186-187`). Single session at a time is
the operating model.
...
| I4 | One `speak()` is one TOKEN-aggregated synthesis context whose generated audio reaches the transport as one buffered, gap-free span; audio still held is discarded on **interruption** | Synthesis gaps became 1.6–4.2 s of real mic silence; the app heard one utterance as several turns. Playing an interrupted prefix would resurrect a turn the caller already cut off. **Does not extend to synthesis failure** — see Gaps and issue #25. | `src/voicebox/agent.py:984-989`, `src/voicebox/processors/generated_utterance_audio_buffer.py:37-79` | `tests/test_generated_utterance_audio_buffer.py`, `tests/test_generated_utterance_in_pipeline.py`, `tests/test_kokoro_playout.py`, S3 |
| I16 | The base TTS class owns the `TTSStartedFrame`/`TTSStoppedFrame` bracket; `stop_frame_timeout_s` must outlast whole-utterance synthesis (120 s vs the 3.0 s default) | Kokoro under TOKEN aggregation yields nothing until the whole utterance is synthesized — measured 19.1 s for six sentences. A premature stop frame disarmed the playout buffer, double-counted `_tts_pending`, and resolved `wait_for_playout` before a sample played (D31) | `src/voicebox/agent.py:984-989` | `tests/test_generated_utterance_in_pipeline.py::test_one_speak_produces_exactly_one_tts_bracket`, S3 |
| I17 | A frame-processor contract involving direction, the start/stop bracket, `StartFrame` propagation, or service configuration is tested through a real pipeline (`pipecat.tests.utils.run_test`), and a test counts only once it has been shown to fail | Four defects shipped or nearly shipped behind a green suite; the hand-driven harness fed an `ErrorFrame` downstream, a direction production never produces (D31, D32, D33) | `CLAUDE.md` "Testing frame processors"; `tests/test_generated_utterance_in_pipeline.py:1-14` | `tests/test_generated_utterance_audio_buffer.py`, issue #24 |
...
| I11 | Artifact dump snapshots `AudioBufferProcessor` buffers BEFORE `stop_recording()` and runs BEFORE `EndFrame` | `stop_recording()` resets the buffers; the processor closes after `EndFrame` — either ordering error loses the WAVs | `src/voicebox/agent.py:580-590`, `src/voicebox/agent.py:658-665` | S4 |
| I12 | Teardown emits `session_stopped` before anything else so pending `listen()`s return it instead of being cancelled; the pipeline ends via `EndFrame`, never `worker.stop()` | `BaseWorker.stop()` never ends the run — awaiting the runner hangs forever | `src/voicebox/agent.py:558-560`, `src/voicebox/agent.py:589-593`, `src/voicebox/bot.py:90-96` | S4 |
| I13 | `speak(when=...)` snapshots the event-log position synchronously at arm time | Capturing it inside the spawned task races the trigger event and can drop it | `src/voicebox/agent.py:816-821` | S3 |
| I14 | App-bot transcripts claim the earliest *unclaimed VAD start* for `turn_started_at`, not the aggregator's stamp | Under batch STT the aggregator stamps arrival time — observed 103 s off | `src/voicebox/agent.py:356-386`, `src/voicebox/agent.py:396-401` | S2 |
| I15 | Shim diagnostics ride the *console* channel (`[voice-shim]`-tagged lines → `shim.log`), not `window.__voiceShim` polling | The diag object is re-created on every navigation (init script runs per document) — a snapshot only describes the final document; the console stream survives navigations | `src/voicebox/browser_session.py:183-202`, `src/voicebox/shim.js:66-70` | `tests/test_browser_session.py::test_shim_artifacts_written`, S4 |

Quality budgets baked into the design (not aspirations — measured constants): VAD stop lag
~1.0 s on every `app_bot_speech_stopped` (`src/voicebox/agent.py:95-99`); Whisper CPU int8
≈0.4–0.55× realtime warm (`src/voicebox/processors/nonblocking_whisper_stt.py:48-55`);
`listen(timeout≤45)` to stay under MCP clients' ~60 s HTTP cap (`src/voicebox/server.py:255-257`).
...
| # | Hop | View | Evidence |
|---|-----|------|----------|
| 1 | MCP tool entry; audio/CDP ports preflighted | development | `src/voicebox/server.py:125-187` |
| 2 | pipecat child spawned with queue pair | process | `src/voicebox/agent_ipc.py:87-96` |
| 3 | Child builds agent + `WebsocketServerTransport` on :9091 | logical | `src/voicebox/bot.py:49-50`, `src/voicebox/agent.py:1042-1046` |
| 4 | Browser child spawned; Chromium launched with CDP port, mic permission, shim init-script on the one page | physical | `src/voicebox/browser_session.py:76-93`, `:273-306` |
| 5 | `goto(url)` then poll until shim installed; failure → `{"ok": false}` → parent raises and tears down the pipecat child | process | `src/voicebox/browser_session.py:308-321`, `src/voicebox/server.py:193-204`, `src/voicebox/browser_session.py:225-253` |
| 6 | Shim opens WS to :9091 (1 s reconnect loop covers pipecat starting late) | process | `src/voicebox/shim.js:94-140` |
| 7 | `on_client_connected` → `client_connected` event in the log | logical | `src/voicebox/agent.py:493-497` |
| 8 | Returns `{cdp_endpoint, audio_ws_url, attach_hint}` for the external UI driver | physical | `src/voicebox/browser_session.py:105-110` |
...
| # | Hop | View | Evidence |
|---|-----|------|----------|
| 1 | MCP `speak` tool → IPC with mode-dependent deadline | development | `src/voicebox/server.py:280-347` |
...
| 5 | LLM-response frame triplet queued → VAD and STT pass it through → stock Kokoro synthesizes one TOKEN context → `GeneratedUtteranceAudioBuffer` releases one contiguous span → retained assistant aggregator records tester context | logical | `src/voicebox/agent.py:_queue_speak_frames`, `src/voicebox/agent.py:984-989`, `src/voicebox/agent.py:1034`, `src/voicebox/processors/generated_utterance_audio_buffer.py:37-79` |
...
| # | Hop | View | Evidence |
|---|-----|------|----------|
| 1 | MCP `stop` tool, 210 s deadline (I5); reap runs in `finally` even if the graceful path fails | development | `src/voicebox/server.py:350-415` |
| 2 | Child loop calls `agent.stop()` first, then drains/cancels in-flight command tasks | process | `src/voicebox/bot.py:90-107` |
| 3 | `session_stopped` emitted first — pending `listen()`s return it (I12); armed barge-ins cancelled | logical | `src/voicebox/agent.py:558-566` |
| 4 | STT drained with backlog-scaled budget (15 s + 1 s/audio-s, cap 180 s); incomplete drain logged, never hangs | process | `src/voicebox/agent.py:574-578`, `src/voicebox/processors/nonblocking_whisper_stt.py:196-223` |
| 5 | Event log settled (transcript frames still hop tasks after the queue empties) | process | `src/voicebox/agent.py:599-612` |
| 6 | Artifacts dumped before `EndFrame` (I11): `events.json`, `metrics.json` via `compute_metrics`, WAVs (buffers snapshotted, tester-left/bot-right stereo merge) | logical | `src/voicebox/agent.py:580-590`, `:614-685`, `src/voicebox/metrics.py:15-78` |
| 7 | `EndFrame` queued; `WorkerRunner.run()` returns (I12) | process | `src/voicebox/agent.py:589-593` |
| 8 | Browser child snapshots `window.__voiceShim` to `shim_diag.json` before closing the context; `shim.log` has been appended live all session (I15) | process | `src/voicebox/browser_session.py:328-331`, `:205-224` |
| 9 | Parent reaps the pipecat child and the browser child, merges the shim artifact paths returned by `stop_browser()` into `artifacts` (they survive a wedged pipecat child — the merge sits in the `finally`), returns `{stopped, artifacts}` | physical | `src/voicebox/server.py:401-414`, `src/voicebox/browser_session.py:145-151`, `src/voicebox/agent_ipc.py:38-67` |
...
| The audio path itself has no pytest coverage — it is verified only by live smoke scripts and dogfood sessions | `scripts/smoke_browser_shim.py:1-18`; CLAUDE.md "Quality checks" | A regression at S1 hops 4–7 or S2 hops 1–4 is invisible to `uv run pytest`. Accepted trade-off (needs a real browser); the smoke script is the compensating control and is now usable as one — it serves its own page (`scripts/blank_page_server.py`), needs no voice app, and exits non-zero when no audio arrives (`scripts/smoke_browser_shim.py:139-142`). |
| A failed synthesis plays its partial audio into the app's microphone — I4's failure half is unimplemented | `src/voicebox/processors/generated_utterance_audio_buffer.py:41-51`; `tests/test_generated_utterance_in_pipeline.py` strict xfail; BUILDLOG D32 | S3 delivers a truncated tester turn the app answers as if complete. Both obvious fixes were traced and rejected (the buffer sits downstream of the upstream-travelling `ErrorFrame`; the `on_error` event races ahead of the in-flight audio). Needs a design pass — [issue #25](https://github.com/IsisChameleon/voicebox/issues/25). |
| `start_browser_session` reports a session ready before the audio WebSocket accepts frames | `scripts/smoke_browser_shim.py` run 2026-08-14: transport bound ~9 s after `start_browser` returned, shim retrying meanwhile | S1 hop 7's `client_connected` is not a usable readiness signal; an utterance spoken in that window is stranded. Now reproducible in-repo in ~25 s with no external app — [issue #22](https://github.com/IsisChameleon/voicebox/issues/22). |
